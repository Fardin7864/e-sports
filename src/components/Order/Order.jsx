...
import { removeFromLocalStorage } from "../LocalStorageManagment/localStorage";

const Order = ({event, handleRender}) => {
    const { title, img, description, price , rating, total_reviews, id} = event;
    ...
    return (
        ...
        <figure className="">
          ...
          <div className=" mt-5">
              <Link onClick={() => handleRemove(id)} className="btn btn-primary w-full bg-gradient-to-r from-[#be006b] to-[#e33b54] text-transparent  bg-clip-text ">Remove </Link>
          </div>
      </div>
    );
};
